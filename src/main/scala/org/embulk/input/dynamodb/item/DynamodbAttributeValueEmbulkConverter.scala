...
object DynamodbAttributeValueEmbulkConverter {
...
case class DynamodbAttributeValueEmbulkConverter(schema: DynamodbItemSchema) {
  ...
  def withAttribute(
    ...
    Convert(
      attributeValue,
      typeEnforcer = schema.getAttributeType(attributeName),
      timestampParser = schema.getTimestampParser(attributeName)
    )
  }
  ...
  case class Convert(
    ...
    def asBoolean: Option[Boolean] = {
      ...
      Option(fromAttributeValueType match {
        case DynamodbAttributeValueType.S =>
          if (DynamodbAttributeValueEmbulkConverter.TRUTHY_STRINGS
                .contains(attributeValue.getS)) true
          else if (DynamodbAttributeValueEmbulkConverter.FALSY_STRINGS
        ...
        case DynamodbAttributeValueType.B =>
          ...
          if (DynamodbAttributeValueEmbulkConverter.TRUTHY_STRINGS.contains(s))
            true
          else if (DynamodbAttributeValueEmbulkConverter.FALSY_STRINGS
                     .contains(s)) false
          else return None
